...
class MyLevel extends Level{
		...
		if($this->nextSave < $now){
			foreach($this->usedChunks as $i => $c){
				if(count($c) === 0){
					...
					$this->level->unloadChunk((int) array_pop($X), (int) $Z, true);
				}
			}
	...
	public function save($force = false, $extra = false){
		if(!isset($this->level)){
			return false;
		}
		$extra = false; //-
...
class MyWorldGenerator{
  ...
  public function __construct(LevelGenerator $generator, $name, $seed=false, $width = 16, $height = 8){
    ...
    $this->level = new MyLevel($level, $entities, $tiles, $blockUpdates, $name);
  }
...
while (count($argv) > 1) {
  ...
  } elseif (substr($argv[0],0,2) == "-I") {
    require_once(substr($array_shift($argv),2));
  } elseif (substr($argv[0],0,9) == "--preset=") {
    $presets = substr(array_shift($argv),9);
  } else {
    break;
  }
}
...
if (is_dir(DATA_PATH.'worlds/'.$world_name))
  die("$world_name: already exists\n");

$pluginApi = new PluginAPI();
$options = array();
if (isset($presets)) $options["preset"] = $presets;
...
$wg = new MyWorldGenerator($generator,$world_name,$seed);
$wg->generate();
$wg->close();
